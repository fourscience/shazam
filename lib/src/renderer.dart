...
class CodeRenderer implements Renderer {
  ...
  Future<void> _emitDocument(
    ...
    final otherRecords = ir.records
        .where((r) => r.owner == null || (!fragmentOwners.contains(r.owner!) && !opRecords.containsKey(r.owner!)))
        .toList();
    ...
    final typesBuilder = LibraryBuilder()
      ..directives
          .addAll(_importsForRecords(otherRecords, fragmentRecordOwners, prefix: 'fragments/'))
    ...
    for (final record in otherRecords) {
      typesBuilder.body.addAll(_emitRecord(record, recordNames, enumNames));
    }
    ...
    for (final op in ir.operations) {
      ...
      final builder = LibraryBuilder()
        ..directives.addAll([
          ...
          ..._importsForRecords(
              opOwned, fragmentRecordOwners, prefix: '../fragments/'),
          ...op.fragments
              .map((f) => Directive.import('../fragments/$f.dart')),
        ])
        ..body.add(Code('\n// Generated by shazam\n'))
      ...
      for (final record in opOwned) {
        builder.body.addAll(_emitRecord(record, recordNames, enumNames));
      }
      for (final plugin in plugins) {
    ...
    for (final frag in ir.fragments) {
      final builder = LibraryBuilder()
        ..directives.addAll(_fragmentImports(
            frag, fragmentRecords, fragmentRecordOwners))
  ...
  List<Spec> _emitRecord(
    ...
    if (record.isInput) {
      final serialize = Method((b) {
        ...
        for (final f in fields) {
          final expr =
              _serializeForType(f.type, 'data.${f.name}', recordNames, enumNames);
...
''';
  ...
  String _deserializeForType(
      String type, String source, Set<String> recordNames, Set<String> enumNames) {
    ...
    if (core.startsWith('List<') && core.endsWith('>')) {
      ...
      final innerExpr =
          _deserializeForType(inner, 'e', recordNames, enumNames);
  ...
  String _serializeForType(
    ...
    if (core.startsWith('List<') && core.endsWith('>')) {
      ...
      final chain = nullable ? '?.' : '.';
      final innerExpr =
          _serializeForType(inner, 'e', recordNames, enumNames);
  ...
  String _enumCase(String name) {
    ...
    final camel = parts
        ...
        .join();
    return camel.isEmpty ? 'value' : camel[0].toLowerCase() + camel.substring(1);
  }
  ...
  List<Directive> _importsForRecords(
    ...
    return owners
        .map((o) => Directive.import('$prefix$o.dart'))
        .toList();
  }
